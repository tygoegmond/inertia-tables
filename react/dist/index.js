    ...
    const columns = React__namespace.useMemo(() => {
        ...
        return configColumns
            ...
            header: ({ column: tanstackColumn }) => {
                ...
                return (jsxRuntime.jsxs("div", { className: `flex items-center gap-2 ${column.sortable ? 'cursor-pointer select-none hover:text-foreground' : ''}`, onClick: () => {
                        if (column.sortable && onSort) {
                            const newDirection = direction === 'asc' ? 'desc' : 'asc';
                            onSort(column.key, newDirection);
                        }
                    }, children: [column.label, column.sortable && (jsxRuntime.jsxs("div", { className: "flex flex-col", children: [jsxRuntime.jsx(ChevronUp, { className: `h-3 w-3 ${isActive && direction === 'asc' ? 'text-foreground' : 'text-muted-foreground'}` }), jsxRuntime.jsx(ChevronDown, { className: `h-3 w-3 -mt-1 ${isActive && direction === 'desc' ? 'text-foreground' : 'text-muted-foreground'}` })] }))] }));
            },
            cell: ({ row }) => {
...
function InertiaTable({ state, className }) {
    ...
    const handleSort = (column, direction) => {
        react.router.get(window.location.pathname, { sort: column, direction }, {
            preserveState: true,
            preserveScroll: true
        });
    };
    const handlePageChange = (page) => {
        react.router.get(window.location.pathname, { page }, {
            preserveState: true,
            preserveScroll: true
        });
    };
